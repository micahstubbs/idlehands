...
$(document).ready(function() {
	...
	utilities.createThree(
		...
		function(scene) {
			...
			worldHandHolder.scale.set(0.8, 0.8, 0.8);
			...
			handHolder.rotateX(Math.PI * 0.7);
			handHolder.position.z -= 40;

			// Create hands
			hands = [
				new Hand(handHolder, worldHandHolder, -1),
				new Hand(handHolder, worldHandHolder, 1)
			];
...
function createPoem() {
	...
	for (var i = 0; i < 5; i++) {
		createOnePoem(i + Math.random() * 0.2);
	}
