...
var Hand = Class.extend({
	...
	update: function(time, scene, camera) {
		...
		if (!this.isLeapControlled) {
			...
			this.v.addSpherical(3, 14 * utilities.noise(time.current * .60 + 10 * this.xMult), 4 * utilities.noise(time.current * .70 + 10 * this.xMult));

			this.pos.addMultiple(this.v, time.elapsed);
			var offset = this.pos.getOffsetTo(this.center);
			var d = offset.magnitude();
			this.v.addMultiple(offset, .002 * d);
			this.v.mult(.98);
			this.v.z *= .85;
...
var Finger = Class.extend({
	init: function(hand, index) {
		...
		var lengthScale = .8 * Math.sin(index * .6 + .45) + .5;
		...
		if (index === 0) {
			...
			this.root.rotateX(Math.PI * .45);
			this.root.rotateY(-.8);

		} else {
			this.root.rotateX(-.2 * (index - 1));
...
var d = p.getDistanceTo(p0);
	...
	update: function(time, scene, camera) {
		...
		for (var i = 0; i < this.joints.length; i++) {
			var curl = utilities.noise(time.current + this.index * .1 + 10 * this.hand.xMult) * .5 + .5;

			//curl = Math.pow(curl, 3);
			curl = utilities.sCurve(curl, 2);


			this.joints[i].rotation.z = .6 * (curl * Math.pow(i + .3, .5));
		}
